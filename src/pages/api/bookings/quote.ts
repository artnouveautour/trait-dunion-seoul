...
import { quoteEmail } from '../../../lib/bookingEmails';
...
export const POST: APIRoute = async (ctx) => {
  ...
  const { id, amount, breakdown, bankInfo, sendNow } = body || {};
  if (!id || !amount) return badRequest('id, amount 필수');
  ...
  const updated: Booking = {
    ...
    quote: {
      ...
      bankInfo: bankInfo ? String(bankInfo) : undefined,
  ...
  if (sendNow) {
    try {
      const tpl = quoteEmail(updated);
      await sendEmail(env, { to: updated.email, subject: tpl.subject, html: tpl.html, text: tpl.text });
    } catch (e: any) {
      return serverError('견적 발송 실패: ' + (e?.message || e));
    }
  }

  await moveBooking(kv, booking.status, 'quoted', updated);
